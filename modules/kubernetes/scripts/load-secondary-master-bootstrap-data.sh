...
aws s3 cp s3://${s3_bootstrap_user_data_bucket}/ ./ --recursive --exclude "*" --include "*master-kubernetes*" --include "*secondary-master-kubernetes*"
...
if [[ -f "secondary-master-kubernetes-bootstrap-script.enc" ]]; then
    cat secondary-master-kubernetes-bootstrap-script.enc | base64 --decode | gunzip > master-kubernetes-bootstrap-script.sh
